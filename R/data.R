#' Effect matrix for the crisp multilevel tutorial
#'
#' @name crispEM
#' @docType data
#' @keywords data
#' @usage data(crisp)
NULL


#' Subject identifier for the rye metabolomics regression tutorial
#'
#' @name IDR
#' @docType data
#' @keywords data
#' @usage data(freelive)
NULL

#' Metabolomics data for the rye metabolomics regression tutorial
#'
#' @name XRVIP
#' @docType data
#' @keywords data
#' @usage data(freelive)
NULL

#' Rye consumption for the rye metabolomics regression tutorial
#'
#' @name YR
#' @docType data
#' @keywords data
#' @usage data(freelive)
NULL


#' Subject identifier for the rye metabolomics regression tutorial (no individuals are repeated twice)
#'
#' @name IDR2
#' @docType data
#' @keywords data
#' @usage data(freelive)
NULL

#' Metabolomics data for the rye metabolomics regression tutorial (no individuals are repeated twice)
#'
#' @name XRVIP2
#' @docType data
#' @keywords data
#' @usage data(freelive)
NULL

#' Rye consumption for the rye metabolomics regression tutorial (no individuals are repeated twice)
#'
#' @name YR2
#' @docType data
#' @keywords data
#' @usage data(freelive)
NULL


#' Microbiota composition in mosquitos for the classification tutorial
#'
#' @name Xotu
#' @docType data
#' @keywords data
#' @usage data(mosquito)
NULL

#' Village of capture of mosquitos for the classification tutorial
#'
#' @name Yotu
#' @docType data
#' @keywords data
#' @usage data(mosquito)
NULL
